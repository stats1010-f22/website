...
#| include: false
# Load your data here
titanic <- read_csv(here("repo-structure/data/titanic.csv"))
```
...
titanic_modified <- 
    titanic %>%
    group_by(Pclass) %>%
    mutate(total_class_fare = sum(Fare),
           Survived_num = ifelse(Survived == "yes", 1, 0))
```

```{r plot-titanic_modified}
#| echo: false
#| eval: true
...
ggplot(data = titanic_modified, 
       mapping = aes(x = Age, 
                     y = Survived_num, 
                     color = total_class_fare)) +
  geom_point() + 
...
#| echo: false
#| eval: true
kable(head(titanic), format = "html")
```

# Some text {background-image="https://www.purina.com.au/-/media/project/purina/main/breeds/dog/dog_boston-terrier_desktop.jpg?h=475&la=en&w=825&hash=DC3F38D08CCE086118327D827C002A62" background-size="cover"}
