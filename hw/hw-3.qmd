...
5.  Perform 3 hypothesis tests to see if any of the variables length ($x$), width ($y$), and depth ($z$) of a diamond are the same. State all hypotheses, p-values, and test statistics, then interpret the results in context. Draw plots suitable for each hypotheses.
