...
It is more accurate because it does two things: 
1. It's  longer wider around $0.5$ than away from $0.5$. 
1. It's better because it lets the variance change along the interval. At p_low it used $p_{low}(1-p_{low})$ and at $p_{high}$ it uses $p_{high}(1-p_{high})$ where as the approximation we use has fixed variance.
